...
class Operator:
    ...
    async def handle_message(
        ...
        config_id = config['id']
        mode = await self.redis.get_mode(config_id)

        if not mode:
            logger.warning(f"Для чата {config_id} не установлен режим. Сообщение проигнорировано.")
            return
