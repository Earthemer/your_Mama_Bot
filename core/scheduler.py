...
class SchedulerManager:
    ...
    async def start(self):
        """
        Настраивает расписание для всех активных чатов и устанавливает
        им начальный режим 'PASSIVE' при старте бота.
        """
        ...
        logger.info(f"Успешно настроено расписание и установлен режим PASSIVE для {len(all_configs)} чатов.")
    ...
    def _schedule_daily_routines(self, config: dict[str, Any]):
        ...
        def schedule_cycle(hour: int, minute: int, duration: int, label: str):
            ...
            self.scheduler.add_job(
                ...
                trigger="cron", hour=hour, minute=minute, second=jitter_seconds,
                timezone=timezone,
                args=[config_id, label], id=f"gathering_{label}_{config_id}", replace_existing=True
            )

            online_start_time = datetime.now(timezone).replace(
                hour=hour, minute=minute, second=jitter_seconds, microsecond=0
            ) + timedelta(minutes=GATHERING_DURATION_MINUTES)
    ...
    async def _run_gathering_start(self, config_id: int, time_of_day: str):
        """Переводит чат в режим GATHERING."""
        logger.debug(f"SCHEDULER: Начало GATHERING '{time_of_day}' для config_id={config_id}")
    ...
    async def _run_processing_and_online_start(self, config_id: int, time_of_day: str, online_duration: int,
                                               timezone: ZoneInfo):
        """Запускает обработку накопленных сообщений и включает ONLINE-режим."""
        logger.debug(f"SCHEDULER: Начало ONLINE '{time_of_day}' для config_id={config_id} на {online_duration} минут")

        # Сначала "мозг" обрабатывает все накопленное и стартует сессию
        await self.brain.start_online_interactions(config_id, time_of_day)

        # Только после этого переключаем режим и планируем следующие шаги
        await self.redis.set_mode(config_id, BotMode.ONLINE.value)

        pulse_job_id = f"online_pulse_{config_id}_{time_of_day}"
        end_job_id = f"online_end_{config_id}_{time_of_day}"
        ...
        self.scheduler.add_job(
            ...
            args=[config_id, pulse_job_id, end_job_id],
            id=end_job_id,
            replace_existing=True
        )

    @log_error
    async def _run_online_end(self, config_id: int, pulse_job_id: str, end_job_id: str):
        """Завершает ONLINE-режим и подчищает временные задачи."""
        logger.info(f"SCHEDULER: Плановое завершение ONLINE для config_id={config_id}")
    ...
    async def _run_random_session_check(
        ...
        if current_mode is not None and current_mode != BotMode.PASSIVE.value:
            logger.debug(
                f"SCHEDULER: Рандомная сессия отменена (бот не в режиме PASSIVE, текущий режим: {current_mode})")
            return

        if random.random() < (chance_percent / 100.0):
            logger.debug(f"SCHEDULER: Рандомная сессия активирована для config_id={config_id}")
            ...
            logger.debug(f"SCHEDULER: Рандомная сессия не активирована для config_id={config_id}")